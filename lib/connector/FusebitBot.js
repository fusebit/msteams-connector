...
class FusebitBot extends TeamsActivityHandler {
    ...
    async getUserContext(context) {
        const s = await context.fusebit.storage.get(this._getStorageIdForTeamsUser(context.activity.from.id));
    ...
    async saveUserContext(userContext) {
        return context.fusebit.storage.put({ data: userContext }, this._getStorageIdForTeamsUser(userContext.teamsUser.user));
    }
...
`,
    ...
    async sendSignInCardAsync(context) {
        ...
        await context.fusebit.storage.put(
            { data: { status: 'authenticating', state, timestamp: Date.now() } },
            this._getStorageIdForTeamsUser(context.activity.from.id)
        );
    ...
    async deleteTeamsUser(context, userContext) {
        ...
        if (userContext) {
            ...
            await context.fusebit.storage.delete(this._getStorageIdForVendorUser(userContext.vendorUserId));
            await context.fusebit.storage.delete(this._getStorageIdForTeamsUser(context.activity.from.id));
        }
    }
