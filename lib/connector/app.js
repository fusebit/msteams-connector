...
exports.createApp = (bot) => {
        ...
        async (req, res, next) => {
            ...
            while (true) {
                const response = await Superagent.get(
                    `${req.fusebit.configuration.fusebit_storage_audience}/v1/account/${req.fusebit.accountId}/subscription/${req.fusebit.subscriptionId}/function`
                )
                ...
                await Promise.all(
                    response.body.items.map((f) => {
                        return Superagent.delete(
                            `${req.fusebit.configuration.fusebit_storage_audience}/v1/account/${req.fusebit.accountId}/subscription/${req.fusebit.subscriptionId}/boundary/${f.boundaryId}/function/${f.functionId}`
                        ).set('Authorization', `Bearer ${req.fusebit.fusebit.functionAccessToken}`);
                    })
                );
            }
    ...
    app.post(
        ...
        async (req, res) => {
            ...
            const userContext = vendor2teams ? await req.fusebit.storage.get(vendor2teams.storageId) : undefined;
            if (!userContext) {
