        ...
        async (req, res, next) => {
            ...
            while (true) {
                const response = await Superagent.get(
                    `${await bot._getBaseUrl(req.fusebit)}/v1/account/${req.fusebit.accountId}/subscription/${req.fusebit.subscriptionId}/function`
                )
                ...
                await Promise.all(
                    response.body.items.map((f) => {
                        return Superagent.delete(
                            `${await bot._getBaseUrl(req.fusebit)}/v1/account/${req.fusebit.accountId}/subscription/${req.fusebit.subscriptionId}/boundary/${f.boundaryId}/function/${f.functionId}`
                        ).set('Authorization', `Bearer ${req.fusebit.fusebit.functionAccessToken}`);
                    })
                );
            }
